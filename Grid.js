module.exports = function(width, height, wrap = false) {
	/*
	...
	_self.eachPointRandom = function(callback) {
		var randXIndices = [];
		var randYIndices = [];
	 ...
	 * @param		{function}	callback		A callback function
	 */
	_self.eachPointReverse = function(callback) {
	...
	_self.eachMetaPointReverse = function(callback) {
		for(var y = _height - 1; y >= 0; y--) {
			for(var x = _width - 1; x >= 0; x--) {
				var point = _meta[y][x];
	...
	_self.setHexValues = function(filterRule = false) {
		_self.eachPoint(function(point, x, y) {
			var value			= 0;
			var numNeighbors	= 0;
			var numCorners		= 0;
	...
	_self.pathfind = function(start, end, bounds = false) {
		var pathPoints	= [];
		var pathFound	= false;
			...
			for(var basePoint of basePoints) {
				...
				for(var dir in meta.neighbors) {
					...
					if( dirValue ) {
						...
						if( directionCoords[dir].diagonal ) {
